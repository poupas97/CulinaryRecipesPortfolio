...
const login = async (req, res) => {
  try {
    ...
    const user = userDtoComplex(
      await UserConnection.singleUserByUsername(username)
    );
...
const authenticateToken = async (req, res, next) => {
  try {
    ...
    if (!needsAuthentication(method, url)) next();
      ...
      if (!token)
        return res.status(401).json(errorDtoSimple(ErrorMapper.UNAUTHORIZED));

      const userAuthenticated = verifyToken(token);
      if (!userAuthenticated) return res.status(403);

      const user = await UserConnection.singleUserByUsername(
...
const refresh = async (req, res) => {
  try {
    const {
      body: { username, password, refreshToken: tokenReceived },
    } = req;
